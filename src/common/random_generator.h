...
class RandGenerator<cpu, DType> {
 public:
  ...
  static const int kMinRndNumberPerThread = 64;
  // store how many global random states for CPU.
  static const int kRndStateNum = 1024;
  ...
  static void AllocState(RandGenerator<cpu, DType> *inst) {
    inst->states_ = new std::mt19937[kRndStateNum];
  }
  ...
  MSHADOW_XINLINE void Seed(Stream<cpu> *, uint32_t seed) {
    for (int i = 0; i < kRndStateNum; ++i) (states_ + i)->seed(seed + i);
  }
...
class RandGenerator<gpu, DType> {
 public:
  ...
  static const int kRndStateNum = 32768;
  ...
  static void AllocState(RandGenerator<gpu, DType> *inst) {
    cudaError_t e = cudaMalloc(&inst->states_,
                               kRndStateNum * sizeof(curandStatePhilox4_32_10_t));
  ...
  static void FreeState(RandGenerator<gpu, DType> *inst) {
    cudaError_t err = cudaFree(inst->states_);
...
class RandGenerator<gpu, double> {
 public:
  ...
  static void AllocState(RandGenerator<gpu, double> *inst) {
    cudaError_t e = cudaMalloc(&inst->states_,
                               kRndStateNum * sizeof(curandStatePhilox4_32_10_t));
    if (e != cudaSuccess && e != cudaErrorCudartUnloading) {
      throw std::bad_alloc();
  ...
  static void FreeState(RandGenerator<gpu, double> *inst) {
    cudaError_t err = cudaFree(inst->states_);
    if (err != cudaSuccess && err != cudaErrorCudartUnloading) {
      LOG(FATAL) << "CUDA: " << cudaGetErrorString(err);
    }
